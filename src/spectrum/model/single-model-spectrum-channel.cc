...
SingleModelSpectrumChannel::GetTypeId()
{
    static TypeId tid = TypeId("ns3::SingleModelSpectrumChannel")
        ...
        // 用不同于 "PropagationLossModel" 的名称
        .AddAttribute("SpectrumPropagationLossModel",
                      "Pointer to a SpectrumPropagationLossModel used by SingleModelSpectrumChannel",
                      PointerValue(nullptr),
                      MakePointerAccessor(&SingleModelSpectrumChannel::m_spectrumLoss),
...
SingleModelSpectrumChannel::StartTx(Ptr<SpectrumSignalParameters> txParams)
{
    ...
    // 遍历接收者
    for (auto rxPhy : m_phyList)
    {
        if (rxPhy != txParams->txPhy)
        {
            Ptr<NetDevice> rxDev = rxPhy->GetDevice();
            Ptr<NetDevice> txDev = txParams->txPhy->GetDevice();

            if (rxDev && txDev &&
                rxDev->GetNode()->GetId() == txDev->GetNode()->GetId())
            {
                // same node => skip
                continue;
            }

            if (m_filter && m_filter->Filter(txParams, rxPhy))
            {
                continue;
            }

            // 拷贝 SpectrumSignalParameters
            Ptr<SpectrumSignalParameters> rxParams = txParams->Copy();

            // 这里可以保留天线增益 / pathLossDb / trace等，但不要对 psd 做乘法
            // ...
            // e.g. double txAntennaGain=..., rxAntennaGain=..., pathLossDb=0 ...
            // do your logs, but no "*(rxParams->psd) *= factor"

            // 计算传播时延
            Time delay = MicroSeconds(0);
            if (m_propagationDelay)
            {
                delay = m_propagationDelay->GetDelay(txParams->txPhy->GetMobility(),
                                                     rxPhy->GetMobility());
            }

            Simulator::ScheduleWithContext(
                rxDev ? rxDev->GetNode()->GetId() : 0,
                delay,
                &SingleModelSpectrumChannel::StartRx,
                this, rxParams, rxPhy);
        }
...
{
    NS_LOG_FUNCTION(this << params);
    // 如果已设置 "SpectrumPropagationLossModel"
    if (m_spectrumLoss != nullptr)
    {
        // 在这里一次性做谱域衰落
        params->psd = m_spectrumLoss->CalcRxPowerSpectralDensity(
            params,
            params->txPhy->GetMobility(),
            receiver->GetMobility());
    }
